        ...
        <div v-for="product in selectedProducts" :key="product.id" class="rounded-lg border bg-card">
            ...
            <div class="flex items-center space-x-2">
              <div class="flex">
                <template v-for="n in 5" :key="n">
                  <svg :class="[n <= Math.floor(product.rating) ? 'text-yellow-400' : 'text-gray-300']"
          ...
          <div class="p-4 border-b">
            ...
            <div class="space-y-2">
              <div v-for="(value, key) in product.specifications" :key="key" 
                class="grid grid-cols-2 gap-2 py-1 border-b last:border-0">
          ...
          <div class="p-4">
            <button @click="addToCart(product.id)"
              class="w-full inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2">
        ...
        <div class="relative bg-card rounded-lg shadow-lg w-full max-w-2xl">
          ...
          <div class="p-4 border-b">
            <div class="relative">
              ...
              <input type="text" v-model="searchQuery" placeholder="Ürün ara..."
                class="w-full pl-10 pr-4 py-2 border rounded-md bg-background focus:outline-none focus:ring-2 focus:ring-primary/50">
            </div>
          </div>

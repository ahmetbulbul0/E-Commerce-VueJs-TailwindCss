        ...
        <div class="flex h-16 items-center justify-between px-4">
            ...
            <div class="flex items-center space-x-4">
                <div class="relative">
                    ...
                    <NotificationsList v-if="isNotificationsOpen" :notifications="notifications" all-notifications-route="/customer/notifications" @mark-as-read="markAsRead" @mark-all-as-read="markAllAsRead" />
                </div>

                <div class="relative">
                    <button @click="isProfileMenuOpen = !isProfileMenuOpen" class="flex items-center space-x-3 rounded-lg p-2 hover:bg-accent">
                        <img src="https://github.com/shadcn.png" alt="Customer" class="h-8 w-8 rounded-full" />
                    ...
                    <div v-if="isProfileMenuOpen" class="absolute right-0 mt-2 w-48 rounded-lg border bg-card py-1 shadow-lg">
                        <router-link :to="{ name: 'customer-profile' }" class="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground">
                        ...
                        <router-link :to="{ name: 'customer-profile-notifications' }" class="flex items-center px-4 py-2 text-sm text-muted-foreground hover:bg-accent hover:text-foreground">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="mr-2">
...
// Bildirim sistemi
const { notifications, isNotificationsOpen, unreadCount, markAsRead, markAllAsRead, addSampleNotifications } = useNotifications("customer");

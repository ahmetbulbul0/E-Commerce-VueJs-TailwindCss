<template>
    <header class="sticky top-0 z-50 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            ...
            <div class="flex flex-1 items-center justify-between space-x-2 md:justify-end">
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <router-link :to="{ name: 'cart' }" class="inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 hover:bg-accent hover:text-accent-foreground h-9 w-9 relative">
                        ...
                        <span v-if="cartItemCount > 0" class="absolute -top-1 -right-1 bg-primary text-white rounded-full text-xs w-4 h-4 flex items-center justify-center">
                            {{ cartItemCount }}
                        </span>
                    </router-link>
        ...
        <div v-show="isMenuOpen" class="fixed inset-0 z-50">
            ...
            <!-- Menü İçeriği -->
            <div class="relative z-50 h-full w-full bg-background/95 backdrop-blur-sm supports-[backdrop-filter]:bg-background/60">
                ...
                <div class="p-4">
                    ...
                    <div class="mt-4 border-t border-border/40 pt-4">
                        <div class="space-y-1">
                            <router-link v-for="item in userMenuItems" :key="item.name" :to="item.to" class="flex items-center rounded-lg px-3 py-4 text-base font-medium transition-colors hover:bg-accent" @click="toggleMenu">
                                {{ item.name }}
                            </router-link>
                            <button v-if="isLoggedIn" @click="handleLogout" class="flex w-full items-center rounded-lg px-3 py-4 text-base font-medium transition-colors hover:bg-accent">Çıkış Yap</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div v-if="isSearchOpen" class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="relative w-full max-w-lg px-4">
                <div class="relative">
                    <input v-model="searchQuery" type="text" placeholder="Ürün ara..." class="h-12 w-full rounded-lg border border-border bg-background px-4 pr-12 text-base outline-none transition-all placeholder:text-muted-foreground focus:border-primary" @keyup.enter="handleSearch" ref="searchInput" />
